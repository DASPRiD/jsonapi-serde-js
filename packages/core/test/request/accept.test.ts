    ...
    describe("AcceptParser", () => {
        ...
        for (const [name, header, expectedMessage] of invalidHeaders) {
            it(`should throw error on ${name}`, () => {
                assert.throws(() => AcceptParser.parse(header), new ParserError(expectedMessage));
            });
        }
    });
    ...
    describe("getAcceptableMediaTypes", () => {
        ...
        it("should throw ParserError on invalid header", () => {
            assert.throws(
                ...
                (err: unknown) =>
                    err instanceof ParserError && err.message.includes("Invalid weight"),
                "Expected ParserError to be thrown",
            );
        });
    });
});
